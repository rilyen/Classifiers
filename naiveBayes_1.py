...
class NaiveBayesClassifier(classificationMethod.ClassificationMethod):
    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, kgrid):
        ...
        for i in range(len(trainingData)):
            ...
            commonPrior[label] += 1  # count occurrence of each label
        ...
        for k in kgrid: # Smoothing parameter tuning loop!
            ...
            for label in self.legalLabels:
                labelCount = commonPrior[label]
                for feat in self.features:
                    "*** YOUR CODE HERE to update conditionalProb and counts using Lablace smoothing ***"
                    # util.raiseNotDefined()
                    conditionalProb[(feat, label)] = (commonConditionalProb[(feat, label)] + k) / (labelCount + 2*k)
    ...
    def calculateLogJointProbabilities(self, datum):
        ...
        for label in self.legalLabels:
            ...
            # util.raiseNotDefined()
            logProb = math.log(self.prior[label])  # start with the log of the prior probability
            for feat, value in datum.items():
                if value > 0:
                    logProb += math.log(self.conditionalProb[(feat, label)])
                else:
                    logProb += math.log(1 - self.conditionalProb[(feat, label)])
    ...
    def findHighOddsFeatures(self, label1, label2):
        ...
        featuresOdds.sort(reverse=True, key=lambda x:x[0])
        featuresOdds = [feat for odds, feat in featuresOdds[:100]]
        return featuresOdds
