...
def enhancedFeatureExtractorDigit(datum):
    ...
    # Pixel sum
    pixel_sum = sum(pixels[y][x] for y in range(height) for x in range(width))
    features['pixel_sum'] = pixel_sum

    # Row sums
    for y in range(height):
        row_sum = sum(pixels[y][x] for x in range(width))
        features[f'row_sum_{y}'] = row_sum

    # Column sums
    for x in range(width):
        col_sum = sum(pixels[y][x] for y in range(width))
        features[f'col_sum_{x}'] = col_sum

    # Density of pixels
    num_non_white_pixels = sum(1 for x in range(width) for y in range(height) if pixels[y][x] > 0)
    density = num_non_white_pixels / (width * height)
    features['density'] = density

    # check for edges (pixels are either 0 or 1, so we can see edges more easily by a simple comparison)
    ...
    # Active pixels above the horizontal center line
    for y in range(1, int(height/2)):
        for x in range(1, width):
            features["above horizontal center", x, y] = int(datum.getPixel(x,y))

    # Active pixels to the right of the vertical center line
    # for x in range(1, int(width/2)):
    #     for y in range(1, height):
    #         features["right vertical center", x, y] = int(datum.getPixel(x,y))
